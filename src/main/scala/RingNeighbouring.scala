package org.ngcdi.sckl.neighbouring
...
trait RingNeighbouring {
  ...
  def locateNeighbours(): Unit = {
    ...
    if (!leftOk) {
      ...
      val leftNgbFuture = context.system
        .actorSelection(
          "akka://" + clusterName + "@" + leftNgbName + ":" + nodePort + "/user/" + digitalAssetName
        )
        .resolveOne()(timeout)
    ...
    if (!rightOk) {
      ...
      val rightNgbFuture = context.system
        .actorSelection(
          "akka://" + clusterName + "@" + rightNgbName + ":" + nodePort + "/user/" + digitalAssetName
        )
        .resolveOne()(timeout)
