...
trait DAController {
  ...
  def sendToLocalView(node:String,flow:String,value:Double,time:String,metric:String):Unit={
    context.actorSelection(lViewPath) ! createRecord(node,flow, value, time,metric)
  }

  def createRecord(node:String,flow:String,value:Double,time:String,metric:String):String = {
    log.debug("New Record TO CREATE  ===>"+node+", "+flow+","+value+", "+time+"<=====")
    val dateStr = time.substring(0,10)
    val rcd = node + "," + "1,"+ flow + ", " + time + "," + value + "," + dateStr + "," + metric
