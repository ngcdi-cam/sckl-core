...
trait MessageParser {
  ...
  def parseResponse(request:String,body:ByteString,data:String):Unit ={ log.debug("DA DOES NOT Parse response...") } //= {
  //  processor(entity)
  //}
  ...
  def parsingBehaviour:Receive = {
    ...
    case ParseResponseBodyExtra(request:String,body:ByteString,data:String,dataExtra:Seq[String]) =>
      parseResponseExtra(request,body,data,dataExtra)
  }

  def parseResponseExtra(request:String,body:ByteString,data:String,dataExtra:Seq[String])={
