...
final case class PostExtraRequest(
    queryUrl: String,
    data: String,
...
object RESTClient {
  def props(server: String, port: Int): Props =
    Props(new RESTClient(server, port))
}
...
class RESTClient(server: String, port: Int) extends ScklActor {
  ...
  val poolClientFlow =
    if (!port.equals("80"))
      Http().cachedHostConnectionPool[Promise[HttpResponse]](server, port)
      ...
      case response @ HttpResponse(StatusCodes.OK, _, _, _) =>
        response.entity.contentType match {
          ...
          case _ =>
            ...
            log.debug(
              "RAW RESPONSE CONTENT TYPE==>" + response.entity.contentType
            )
  ...
  def pushAction(
    ...
    val requestNotification = HttpRequest(
      ...
      uri = url,
      entity =
        HttpEntity(ContentTypes.`application/json`, jsonReroute.getBytes())
    ).withHeaders(RawHeader("X-Access-Token", "access token"))

    val responseFuture: Future[HttpResponse] =
      Http().singleRequest(requestNotification)
          ...
          case ContentTypes.`application/json` =>
            response.entity.dataBytes.runFold(ByteString(""))(_ ++ _).foreach {
              body =>
                ...
                replyTo ! ParseResponseBodyExtra(
                  queryUrl,
                  body,
                  data,
                  dataExtra
