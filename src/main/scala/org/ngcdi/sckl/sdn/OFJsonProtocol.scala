...
object OFJsonProtocol extends DefaultJsonProtocol {
  ...
  implicit object matchRuleJsonFormat extends RootJsonFormat[MatchRule] {
    override def read(json: JsValue): MatchRule = json.asJsObject.getFields("in_port", "dl_src", "dl_dst", "dl_type", "nw_src") match {
      case Seq(JsNumber(in_port), JsString(dl_src), JsString(dl_dst), JsNumber(dl_type)) =>
        log.debug("Rule1-->"+in_port+", "+dl_src+", "+dl_dst+", "+dl_type+"<-")
        MatchRule( Option(in_port.toInt), Option(dl_src), Option(dl_dst), Option(dl_type.toInt),  Option("nw_src"))
      ...
      case Seq(JsNumber(in_port),JsString(dl_src), JsString(dl_dst)) =>
        ...
        MatchRule(Option(in_port.toInt), Option(dl_src), Option(dl_dst), None,None)
      case Seq(JsNumber(in_port),JsString(dl_dst), JsNumber(dl_type)) =>
        log.debug("Rule5-->"+in_port+", "+Option(dl_dst).get+", "+dl_type+"<--")
        MatchRule(Option(in_port.toInt), None, Option(dl_dst), Option(dl_type.toInt),None)
      case Seq(JsNumber(in_port), JsNumber(dl_type),JsString(nw_src)) =>
        log.debug("Rule6-->"+in_port+", "+dl_type+","+nw_src+"<--")
        MatchRule(Option(in_port.toInt), None, None, Option(dl_type.toInt),Option(nw_src))
      case x:Any =>
        log.debug("Rule7-->"+x+"<--")
        null
      case _ =>
  ...
  implicit object FlowStatJsonFormat extends RootJsonFormat[FlowStat] {
    ...
    def read(value: JsValue) = {
      value.asJsObject.getFields("byte_count", "packet_count", "duration_sec", "actions", "match") match {
        case Seq(JsNumber(byte_count), JsNumber(packet_count), JsNumber(duration_sec), JsArray(actions), matchr) =>
          new FlowStat(byte_count.toLong, duration_sec.toLong, packet_count.toLong, actions.map(_.convertTo[String]).toSeq, matchr.convertTo[MatchRule])
...
implicit object PortStatJsonFormat extends RootJsonFormat[PortStat] {
  ...
  def read(value: JsValue) = {
    value.asJsObject.getFields("rx_bytes", "tx_bytes", "duration_sec", "port_no") match {
      case Seq(
        //JsNumber(collisions),
        ...
        val o = new PortStat(rx_bytes.toLong, tx_bytes.toLong,duration_sec.toLong, port_no.toString)
        o
      case Seq(
        //JsNumber(collisions),
        //JsNumber(duration_nsec),
        ...
        val o = new PortStat(rx_bytes.toLong, tx_bytes.toLong,duration_sec.toLong, port_no.toInt+"")
        o
      case x:Any =>
